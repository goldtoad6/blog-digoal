...
2、给 PostgreSQL 最新的稳定分支找 bug, 赚取来自 PG 社区的金色小勋章(很多人都想要, 这可是 PG 社区身份的象征).  
  
**太长不看: 写 SKILL 和做规划的部分采用了较为复杂的国产模型 minimax M3; 写代码、review代码、写测试用例和测试、找BUG、写用户手册的部更耗费时间和Token, 则采用了高效且廉价的 stepfun step-3.7-flash ; 最终呈现堪称完美.**  
  
还有羊毛科一薅, 目前 stepfun 在搞免费试用( https://platform.stepfun.com?invite_code=SIASEDCW ), 薅一下给小龙虾续命, 最长免费续命3个月.  
...
这要是直接丢给开发谁受得了, 还不得直接开骂. 但是, 咱有3个高级 SKILL (对skill有兴趣可访问如下网址: https://github.com/digoal/blog/tree/master/skills ), 3步完成.  
...
### 2、给 PostgreSQL 最新的稳定分支找 bug  
  
```  
# 克隆代码  
mkdir ~/new  
cd ~/new  
# 克隆时带上最近600次提交, 因为越新的代码越有机会找到bug  
...
从官网了解到 Step 3.7 Flash 是面向生产级 Agent 的高效率 Flash 模型，为 Agent、Coding、Search 与多模态工作流而生，开放、可部署。 
