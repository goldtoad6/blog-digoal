...
国产模型干不了大活? 是你的姿势不对! 今天用万字长文教你用国产模型 + SKILL干大活.  
  
作为非编程出身的普通AI用户, 不需要科学上网, 不使用国外的模型, 不使用ruflo、Loop等高级编程工具(将复杂任务拆解后调度多个agent分工合作)的情况下, 能不能搞定编程大活?  
...
从简单几句话的需求描述开始, 设计详细的PRD, 从开发架构师的角度编写详细的开发设计文档, 然后开发者开始coding, 代码审查人员开始审查代码, 测试人员编写测试用例并测试新功能, 修复bug, 生成用户手册.  
  
下面演示2个场景, 如何用国产模型干大活:  
  
1、给目前最流行的 PostgreSQL 向量数据库插件 pgvector 增加 8bit 量化功能.  
  
2、给 PostgreSQL 找 bug, 赚取来自 PG 社区的金色小勋章(这个小勋章很多人都想要, 是 PG 社区身份的象征).  
  
**太长不看: 写 SKILL 和做规划的部分采用了较为复杂的国产模型 minimax M3; 写代码、review代码、写测试用例和测试、找BUG、写用户手册的部更耗费时间和Token, 则采用了高效且廉价的 step-3.7-flash ; 最终呈现堪称完美. 多模态能力方面, 图片风格的理解和输出性能 step 要比 minimax M3 更好, 但是对比 M3 输出的 html 页面可用看出 step 深度思考和推理方面似乎略输一筹, 不过这个可以给出更详细的提示词来弥补.**  
  
还有高级羊毛可以薅, 目前 step 在搞免费试用( https://platform.stepfun.com?invite_code=SIASEDCW ), 薅一下给小龙虾续命, 最长可免费续命3个月.  
...
这要是直接丢给开发谁受得了, 还不得直接开骂. 但是, 咱有3个高级 SKILL (对 skill 有兴趣可访问如下网址: https://github.com/digoal/blog/tree/master/skills ), 3步完成.  
...
# 进入项目, 启动 claude, 如果你嫌授权麻烦, 可用开启危险模式.  
cd ~/proj_pgvector_8bit  
...
这个找 bug 的任务其实是比较开放式的, 再加上 PG 以稳如狗著称, 我真没想到 step 的标准模型能找到 bug.  
...
现在的国产大模型对多模的理解能力到底怎么样了, 下面的2个测试我使用苹果官网和某国产数据库官网的截图来告知模型设计风格, 然后让模型根据需求设计 WEB 页面. 评估模型的图片理解能力、编码能力、网络搜索和总结能力.  
...
M3 这里卡顿了有五六分钟的样子才开始输出, 我还以为 hang 了所以按了 ecs 结果还是没响应.  
...
在图片风格的理解和输出性能稳定性这块 step 要比 minimax M3 更好, 但是对比 M3 输出的 html 页面可用看出 step 深度思考和推理方面似乎略输一筹, 不过这个可以给出更详细的提示词来弥补.  
